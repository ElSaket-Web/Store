...
var pluginGtmServerSide = {
  ...
  pushSelectItem: function (item, custom) {
    if (custom?.pagetype) {
      // phpcs:ignore WordPress.WhiteSpace.OperatorSpacing.NoSpaceBefore, WordPress.WhiteSpace.OperatorSpacing.NoSpaceAfter
      this._pushToDataLayer(item, "select_item", custom.pagetype, {
  ...
  _pushToDataLayer: function (
    ...
    for (var item_loop of item) {
      ...
      item_loop.quantity = item_loop.quantity
        ? parseInt(item_loop.quantity, 10)
        : 1;
    ...
    if (
      "add_to_cart" === event &&
      "yes" === varGtmServerSide.is_custom_event_name
    ) {
      this._pushWithStateCartData(eventData);
